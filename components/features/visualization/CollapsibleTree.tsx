...
export interface HierarchyNode {
  ...
  level?: number;
  collapsed?: boolean;
  // Internal flag to mark invisible container root
...
export default function CollapsibleTree({ 
  ...
  const buildHierarchy = useCallback((nodes: HierarchyNode[]): HierarchyNode => {
    // Create a synthetic root node to contain all top-level nodes (hidden in UI)
    ...
    // If there's exactly one top-level node, use it as the visible root
  ...
  useEffect(() => {
    ...
    // Draw links (skip links originating from synthetic root)
    g.selectAll('.link')
      .data(treeData.links().filter(l => !(l.source.data as any).isSyntheticRoot))
      .enter()
      .append('path')
      .attr('class', 'link')
      .attr('fill', 'none')
  ...
  // Add circles for nodes (skip synthetic root)
  nodes.append('circle')
      .attr('r', d => {
    if (d.data.isSyntheticRoot) return 0; // hide synthetic root circle
      ...
      .attr('fill', d => {
  if (d.data.isSyntheticRoot) return 'transparent';
        // Loading state
  ...
  if (d.data.isSyntheticRoot) return 'transparent';
      ...
      .attr('stroke-width', d => {
  if (d.data.isSyntheticRoot) return 0;
  ...
  if (d.data.isSyntheticRoot) return null;
  ...
  nodes.append('text')
    ...
    nodes.append('text')
      ...
      .style('fill', '#333')
      .text(d => (d.data.isSyntheticRoot ? '' : d.data.title))
      .each(function(d) {
        // Wrap long text
        const text = d3.select(this);
        const words = (d.data.isSyntheticRoot ? '' : d.data.title).split(/\s+/);
        if (words.length > 3) {
    ...
    nodes.on('click', (event, d) => {
      ...
      if (node.data.isSyntheticRoot) return; // ignore clicks on synthetic root
  ...
  return (
    <div className="w-full h-full overflow-hidden bg-white rounded-lg border relative">
      ...
      {/* Show infinite tree status */}
      {error && (
        <div className="absolute bottom-4 left-4 text-sm text-red-600 bg-red-50 px-2 py-1 rounded border border-red-200">
          Tree Error: {error}
        </div>
      )}
    </div>
